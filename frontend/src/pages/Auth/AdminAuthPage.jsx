...
const AdminAuthPage = () => {
  ...
  const quickLogin = async () => {
    setForm({ ...form, email: 'admin@learnbridge.com', password: 'admin123' });
    setLoading(true);
    setError('');
    setSuccess('');
    
    try {
      const data = await login({ email: 'admin@learnbridge.com', password: 'admin123' });
      console.log('Login response:', data);
      setToken(data.token);
      setUser(data.user);
      setSuccess('✅ Login successful! Redirecting...');
    ...
    <div className="min-h-screen bg-slate-950 text-slate-50">
      ...
      <div className="flex items-center justify-center px-4 pt-24">
        <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-xl">
          <h1 className="mb-1 text-xl font-bold text-slate-50">Admin Access</h1>
          <p className="mb-4 text-xs text-slate-400">
            Secure access for institution admins to manage users, courses, internships, and analytics.
          </p>
          
          {/* Default Credentials Hint */}
          <div className="mb-4 rounded-lg bg-blue-500/10 border border-blue-500/30 p-3 text-xs">
            <div className="flex items-center justify-between mb-2">
              <p className="font-semibold text-blue-300">🔑 Default Admin Login:</p>
              <button
                onClick={quickLogin}
                disabled={loading}
            ...
            <p className="text-blue-200">Email: admin@learnbridge.com</p>
